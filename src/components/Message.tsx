...
const Message: React.FC<MessageProps> = ({ message, streamingText, isMobile }) => {
        ...
        <ReactMarkdown
          components={{
            ...
            code: ({ node, className, children, ...props }: any) => {
              ...
              return isInline ? 
                <code className="bg-gray-100 dark:bg-zinc-800 rounded px-1 py-0.5" {...props}>{children}</code> :
                <code className="block bg-gray-100 dark:bg-zinc-800 rounded p-2 my-2 overflow-x-auto" {...props}>{children}</code>;
            },
